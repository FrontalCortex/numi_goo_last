...
object AbacusFrameRenderer {
    ...
    private val ORIG_STAR  = Color.parseColor("#ddff33")
    ...
    private fun buildFrameBg2Drawable(context: Context, colorsOverride: IntArray? = null): Drawable {
        ...
        val res = BitmapDrawable(context.resources, bitmap)
        res.setFilterBitmap(true)
        return res
    }
    ...
    private fun buildFrameBg3Drawable(context: Context, colorsOverride: IntArray? = null): Drawable {
        ...
        val rOrig = IntArray(4); val gOrig = IntArray(4); val bOrig = IntArray(4)
        for(k in 0..3) {
            rOrig[k] = (origColors[k] ushr 16) and 0xFF
            gOrig[k] = (origColors[k] ushr 8) and 0xFF
            bOrig[k] = origColors[k] and 0xFF
        }

        // The 3 valid edges (pairs of colours blending into each other)
        // 0: Wood(0)-Star(1),  1: Wood(0)-Black(2),  2: Black(2)-Dark(3)
        val edges = intArrayOf(0, 1,  0, 2,  2, 3)
    ...
    private fun buildFrameBg4Drawable(context: Context, colorsOverride: IntArray? = null): Drawable {
        ...
        // Dış siyah sınır: 0,0 - 400,400 (radius 40)
        paint.color = android.graphics.Color.BLACK
        canvas.drawRoundRect(0f, 0f, 400f, 400f, 40f, 40f, paint)

        // Gradient frame: 5.6,5.6 - 394.4,394.4 (radius 34.4)
        ...
        canvas.drawRoundRect(5.6f, 5.6f, 394.4f, 394.4f, 34.4f, 34.4f, paint)

        // İç siyah sınır: 28,28 - 372,372 (radius 12)
        paint.shader = null
        paint.color = android.graphics.Color.BLACK
        canvas.drawRoundRect(28f, 28f, 372f, 372f, 12f, 12f, paint)

        // İç arka plan: 33.6,33.6 - 366.4,366.4 (radius 6.4)
        paint.color = android.graphics.Color.parseColor("#212226")
        canvas.drawRoundRect(33.6f, 33.6f, 366.4f, 366.4f, 6.4f, 6.4f, paint)

        bitmap.density = context.resources.displayMetrics.densityDpi
        val res = android.graphics.drawable.BitmapDrawable(context.resources, bitmap)
        res.setFilterBitmap(true)
        return res
    }
}
